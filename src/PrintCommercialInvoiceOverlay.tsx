...
export const PrintCommercialInvoiceOverlay = ({ AppContext }) => {
  ...
  return (
    <div className="print-safe-modal fixed inset-0 bg-slate-900/80 z-50 flex flex-col items-center overflow-auto pt-10 px-4 sm:px-0 pb-20 no-print">
      <style>{`
        @media print { 
      ...
      <div id="a4-print-area" className="flex flex-col items-center w-full pb-20">
        <div className="a4-page bg-white shadow-2xl border border-slate-200 relative flex flex-col w-[8.5in] min-h-[297mm] p-[15mm] box-border mb-8">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6 pb-6 border-b-2 border-slate-800">
             <div>
                ...
                <div className="text-xs text-black mt-2 space-y-0.5 whitespace-pre-wrap">
                  {formatAddress({ 
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-4 mb-6">
            <div className="p-3 border border-slate-200">
              <div className="text-xs font-bold text-black uppercase mb-1">Shipper / Exporter</div>
              <div className="font-bold text-sm">{declCompany.name}</div>
              <div className="text-xs text-black whitespace-pre-wrap mt-1">
                {formatAddress({ 
                  line1: declCompany.companyAddress, 
            ...
            <div className="p-3 border border-slate-200">
              <div className="text-xs font-bold text-black uppercase mb-1">For Account & Risk Of (Consignee)</div>
          ...
          <div className="grid grid-cols-4 border border-slate-800 mb-8 divide-x divide-slate-800 text-sm">
             <div className="p-2 flex flex-col">
               <span className="text-[10px] font-bold text-black uppercase">Vessel / Voyage</span>
               <span className="font-semibold mt-1">{ci.vessel || '-'} {ci.voyage || ''}</span>
             </div>
             <div className="p-2 flex flex-col">
               <span className="text-[10px] font-bold text-black uppercase">Port of Loading</span>
               <span className="font-semibold mt-1">{ci.pol || '-'}</span>
             </div>
             <div className="p-2 flex flex-col">
               <span className="text-[10px] font-bold text-black uppercase">Port of Discharge</span>
               <span className="font-semibold mt-1">{ci.pod || '-'}</span>
             </div>
             <div className="p-2 flex flex-col">
               <span className="text-[10px] font-bold text-black uppercase">Incoterms</span>
          ...
          <table className="w-full text-left border border-slate-800 text-sm mb-6">
            <thead className="bg-white border-b border-slate-800">
              <tr>
              ...
              {(ci.lines || []).map((line, idx) => {
                 return (
                   <tr key={idx}>
                     ...
                     <td className="p-2 border-r border-slate-800 text-right">{line.qty} <span className="text-xs text-black">{line.uom}</span></td>
            ...
            <tfoot>
              <tr className="border-t-2 border-slate-800">
                ...
                <td className="p-2 text-right font-bold text-base bg-white">
